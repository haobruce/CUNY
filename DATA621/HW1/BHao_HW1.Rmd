...
* Let's start with a backward elimination by including all variables (except we'll remove -INDEX, -TARGET_WINS_Ratio, -TEAM_BATTING_HBP on the first pass since these are either irrelevant or captured in other columns) and then remove variables until we are left with only statistically significant variables    
...
* Time to return to the original model, but we'll remove the most collinear variables based on the cross correlation and variance inflation factors shown below:  
    - TEAM_BATTING_HR: 0.969 correlation with TEAM_PITCHING_HR  
    - FIELDING_OUTLIER_YN & TEAM_H_Ratio: both are highly correlated to TEAM_FIELDING_E and to each other  
